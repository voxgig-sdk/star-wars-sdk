...
func NewPlanetEntity(client *core.StarWarsSDK, entopts map[string]any) *PlanetEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
// argument it returns the current data as an Planet; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *PlanetEntity) DataTyped(data ...Planet) Planet {
	if len(data) > 0 {
		return typedFrom[Planet](e.Data(asMap(data[0])))
	}
	return typedFrom[Planet](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Planet (all fields
// optional at the wire level).
func (e *PlanetEntity) MatchTyped(match ...Planet) Planet {
	if len(match) > 0 {
		return typedFrom[Planet](e.Match(asMap(match[0])))
	}
	return typedFrom[Planet](e.Match())
}
...
func (e *PlanetEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
// PlanetLoadMatch and returns an Planet. It delegates to the untyped
// Load (identical runtime) and converts at the typed boundary.
func (e *PlanetEntity) LoadTyped(reqmatch PlanetLoadMatch, ctrl map[string]any) (Planet, error) {
	res, err := e.Load(asMap(reqmatch), ctrl)
	if err != nil {
		return Planet{}, err
	}
	return typedFrom[Planet](res), nil
}
...
// PlanetListMatch and returns []Planet. It delegates to the untyped
// List (identical runtime) and converts at the typed boundary.
func (e *PlanetEntity) ListTyped(reqmatch PlanetListMatch, ctrl map[string]any) ([]Planet, error) {
	res, err := e.List(asMap(reqmatch), ctrl)
	if err != nil {
		return nil, err
	}
	return typedSliceFrom[Planet](res), nil
}



func (e *PlanetEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *PlanetEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *PlanetEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {

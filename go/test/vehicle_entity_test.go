...
func TestVehicleEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set STAR_WARS_TEST_VEHICLE_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		vehicleRef01MatchDt0 := map[string]any{
			"id": vehicleRef01Data["id"],
		}
		vehicleRef01DataDt0Loaded, err := vehicleRef01Ent.Load(vehicleRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		vehicleRef01DataDt0LoadResult := core.ToMapAny(entityData(vehicleRef01DataDt0Loaded))
		if vehicleRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if vehicleRef01DataDt0LoadResult["id"] != vehicleRef01Data["id"] {
...
func vehicleBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("STAR_WARS_TEST_VEHICLE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["STAR_WARS_TEST_VEHICLE_ENTID"])

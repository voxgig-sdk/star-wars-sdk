...
func TestPeopleListEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.PeopleList(nil)
		if ent == nil {
			t.Fatal("expected non-nil PeopleListEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := people_listBasicSetup(nil)
		...
		for _, _op := range []string{} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "people_list." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set STAR_WARS_TEST_PEOPLE_LIST_ENTID JSON to run live")
			return
		}
		...
		_ = peopleListRef01Data

	})
}

func people_listBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "people_list", "PeopleListTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read people_list test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse people_list test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("STAR_WARS_TEST_PEOPLE_LIST_ENTID")
	...
	idmapResolved := core.ToMapAny(env["STAR_WARS_TEST_PEOPLE_LIST_ENTID"])

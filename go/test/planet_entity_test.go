...
func TestPlanetEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set STAR_WARS_TEST_PLANET_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		planetRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.planet", setup.data)))
		var planetRef01Data map[string]any
		if len(planetRef01DataRaw) > 0 {
			planetRef01Data = core.ToMapAny(planetRef01DataRaw[0][1])
		}
		...
		_, planetRef01ListOk := planetRef01ListResult.([]any)
		...
		if planetRef01DataDt0Loaded == nil {
...
func planetBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("STAR_WARS_TEST_PLANET_ENTID")
	...
	idmapResolved := core.ToMapAny(env["STAR_WARS_TEST_PLANET_ENTID"])

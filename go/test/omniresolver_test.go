...
import (
	...
	omni "github.com/voxgig-sdk/star-wars-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.StarWarsError); is {
				out := map[string]any{
					"name":    "StarWarsError",
...
func omniCtx(arg any, client *sdk.StarWarsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.StarWarsSDK); is {
			client = live

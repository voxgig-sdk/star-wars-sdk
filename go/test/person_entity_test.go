...
func TestPersonEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		personRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.person")))
...
func personBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("STAR_WARS_TEST_PERSON_ENTID")
	...
	idmapResolved := core.ToMapAny(env["STAR_WARS_TEST_PERSON_ENTID"])

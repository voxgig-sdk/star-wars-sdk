...
func TestSpeciesEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Species(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Species(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set STAR_WARS_TEST_SPECIES_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		speciesRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.species")))
		...
		// LOAD
		speciesRef01MatchDt0 := map[string]any{
			"id": speciesRef01Data["id"],
		}
		speciesRef01DataDt0Loaded, err := speciesRef01Ent.Load(speciesRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		speciesRef01DataDt0LoadResult := core.ToMapAny(entityData(speciesRef01DataDt0Loaded))
		if speciesRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if speciesRef01DataDt0LoadResult["id"] != speciesRef01Data["id"] {
...
func speciesBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("STAR_WARS_TEST_SPECIES_ENTID")
	...
	idmapResolved := core.ToMapAny(env["STAR_WARS_TEST_SPECIES_ENTID"])

...
The Lua SDK for the StarWars API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Film(nil):list(nil, nil)
...
```lua
local result, err = client:Film(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:StarWars(nil):load(
  { id = "test01" }, nil
)

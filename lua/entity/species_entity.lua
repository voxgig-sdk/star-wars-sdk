...
local SpeciesEntity = {}
SpeciesEntity.__index = SpeciesEntity


function SpeciesEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SpeciesEntity)
  self._name = "species"
  self._client = client
...
function SpeciesEntity:get_name()
  return self._name
end


function SpeciesEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SpeciesEntity.new(self._client, opts)
end
...
function SpeciesEntity:mark_deleted()
  self._deleted = true
end


function SpeciesEntity:deleted()
  return true == self._deleted
end


function SpeciesEntity:data_set(args)
  if args ~= nil then
...
function SpeciesEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SpeciesEntity:match_set(args)
  if args ~= nil then
...
function SpeciesEntity:match_get()
...
function SpeciesEntity:stream(action, args, callopts)
...
---@param reqmatch SpeciesLoadMatch
---@param ctrl? table
---@return Species
---@return string? err
function SpeciesEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function SpeciesEntity:_run_op(ctx, post_done)
  local utility = self._utility

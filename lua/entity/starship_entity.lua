...
local StarshipEntity = {}
StarshipEntity.__index = StarshipEntity


function StarshipEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StarshipEntity)
  self._name = "starship"
  self._client = client
...
function StarshipEntity:get_name()
  return self._name
end


function StarshipEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StarshipEntity.new(self._client, opts)
end
...
function StarshipEntity:mark_deleted()
  self._deleted = true
end


function StarshipEntity:deleted()
  return true == self._deleted
end


function StarshipEntity:data_set(args)
  if args ~= nil then
...
function StarshipEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StarshipEntity:match_set(args)
  if args ~= nil then
...
function StarshipEntity:match_get()
...
function StarshipEntity:stream(action, args, callopts)
...
function StarshipEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
function PlanetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PlanetEntity)
  self._name = "planet"
  self._client = client
...
function PlanetEntity:get_name()
  return self._name
end


function PlanetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PlanetEntity.new(self._client, opts)
end
...
function PlanetEntity:mark_deleted()
  self._deleted = true
end


function PlanetEntity:deleted()
  return true == self._deleted
end


function PlanetEntity:data_set(args)
  if args ~= nil then
...
function PlanetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PlanetEntity:match_set(args)
  if args ~= nil then
...
function PlanetEntity:match_get()
...
function PlanetEntity:stream(action, args, callopts)
...
function PlanetEntity:_run_op(ctx, post_done)
  local utility = self._utility

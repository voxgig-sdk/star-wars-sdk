...
local PersonEntity = {}
PersonEntity.__index = PersonEntity


function PersonEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PersonEntity)
  self._name = "person"
  self._client = client
...
function PersonEntity:get_name()
  return self._name
end


function PersonEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PersonEntity.new(self._client, opts)
end
...
function PersonEntity:mark_deleted()
  self._deleted = true
end


function PersonEntity:deleted()
  return true == self._deleted
end


function PersonEntity:data_set(args)
  if args ~= nil then
...
function PersonEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PersonEntity:match_set(args)
  if args ~= nil then
...
function PersonEntity:match_get()
...
function PersonEntity:stream(action, args, callopts)
...
---@param reqmatch PersonLoadMatch
---@param ctrl? table
---@return Person
---@return string? err
function PersonEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PersonListMatch
---@param ctrl? table
---@return Person[]
---@return string? err
function PersonEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function PersonEntity:_run_op(ctx, post_done)
  local utility = self._utility

...
describe("PersonDirect", function()
  it("should direct-list-person", function()
    local setup = person_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-person", setup.live and "live" or "unit")
  ...
  it("should direct-load-person", function()
    local setup = person_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-person", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function person_direct_setup(mockres)

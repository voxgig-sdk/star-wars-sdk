...
describe("PlanetDirect", function()
  it("should direct-list-planet", function()
    local setup = planet_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-planet", setup.live and "live" or "unit")
  ...
  it("should direct-load-planet", function()
    local setup = planet_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-planet", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function planet_direct_setup(mockres)

...
describe("PersonEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Person(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Person(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set STAR_WARS_TEST_PERSON_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    local person_ref01_data_dt0_load_result = helpers.to_map(type(person_ref01_data_dt0_loaded) == 'table' and person_ref01_data_dt0_loaded.data_get and person_ref01_data_dt0_loaded:data_get() or person_ref01_data_dt0_loaded)
...
function person_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("STAR_WARS_TEST_PERSON_ENTID")

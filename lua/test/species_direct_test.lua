...
describe("SpeciesDirect", function()
  it("should direct-list-species", function()
    local setup = species_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-species", setup.live and "live" or "unit")
  ...
  it("should direct-load-species", function()
    local setup = species_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-species", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function species_direct_setup(mockres)

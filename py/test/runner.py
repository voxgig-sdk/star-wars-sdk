...
class StarWarsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            StarWarsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = StarWarsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = StarWarsTestRunner.getenv("STAR_WARS_TEST_LIVE")
        override = StarWarsTestRunner.getenv("STAR_WARS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = StarWarsTestRunner.getenv(key)
        ...
        explain = StarWarsTestRunner.getenv("STAR_WARS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["STAR_WARS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if StarWarsTestRunner._test_control is not None:
            return StarWarsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                StarWarsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            StarWarsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = StarWarsTestRunner.load_test_control()
...
def load_env_local():
    StarWarsTestRunner.load_env_local()


def env_override(m):
    return StarWarsTestRunner.env_override(m)


def entity_data(v):
    return StarWarsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return StarWarsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return StarWarsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return StarWarsTestRunner.load_test_control()


def live_client_options():
    return StarWarsTestRunner.live_client_options()


def live_delay_ms():
    return StarWarsTestRunner.live_delay_ms()

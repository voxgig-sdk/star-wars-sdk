...
class FtCtrl extends StarWarsControl
{
...
class FtClient
{
    ...
    private StarWarsUtility $utility;

    public function __construct(StarWarsUtility $utility, array $options)
    {
    ...
    public function get_utility(): StarWarsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (StarWarsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public StarWarsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (StarWarsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = StarWarsConfig::make_feature($name);
    ...
    private static function buildUrl(StarWarsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof StarWarsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new StarWarsResponse($fetched);
        }
    ...
    private function fail(StarWarsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (StarWarsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

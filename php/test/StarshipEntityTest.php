...
class StarshipEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = StarWarsSDK::test(null, null);
        $ent = $testsdk->Starship(null);
    ...
    public function test_stream(): void
    {
        ...
        $base = StarWarsSDK::test($seed, null);
        $seen = iterator_to_array($base->Starship(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Starship(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = starship_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "starship." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set STAR_WARS_TEST_STARSHIP_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $starship_ref01_data = null;
        if (count($starship_ref01_data_raw) > 0) {
...
function starship_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/starship/StarshipTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["starship01", "starship02", "starship03"] as $k) {
    ...
    $entid_env_raw = getenv("STAR_WARS_TEST_STARSHIP_ENTID");

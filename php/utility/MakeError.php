...
class StarWarsMakeError
{
    public static function call(?StarWarsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new StarWarsContext([], null);
        }
        $op = $ctx->op ?? new StarWarsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new StarWarsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof StarWarsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new StarWarsError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof StarWarsError) {

...
//     (\`new StarWarsSDK(...)\` / \`StarWarsSDK.test(...)\` become
//     \`StarWarsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"film":{"test01":{"id":"test01"}},"people_list":{"test01":{"id":"test01"}},"person":{"test01":{"id":"test01"}},"planet":{"test01":{"id":"test01"}},"species":{"test01":{"id":"test01"}},"starship":{"test01":{"id":"test01"}},"vehicle":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new StarWarsSDK(...)\` and any
// \`StarWarsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+StarWarsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/StarWarsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.STARWARS_TEST_LIVE ||
    'TRUE' === process.env.STARWARS_TEST_OVERRIDE
  ) {
  ...
  m.STARWARS_TEST_EXPLAIN = process.env.STARWARS_TEST_EXPLAIN || m.STARWARS_TEST_EXPLAIN

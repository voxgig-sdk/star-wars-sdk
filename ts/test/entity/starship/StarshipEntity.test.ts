...
describe('StarshipEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'starship.' + op, live)) return
    }
    ...
    const starship_ref01_list = await starship_ref01_ent.list(starship_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['STAR_WARS_TEST_STARSHIP_ENTID']

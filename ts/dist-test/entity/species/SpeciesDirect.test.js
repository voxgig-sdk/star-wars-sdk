...
(0, node_test_1.describe)('SpeciesDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-species', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-species', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-species', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-species', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.STAR_WARS_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    const calls = [];
    const env = (0, utility_1.envOverride)({
        'STAR_WARS_TEST_SPECIES_ENTID': {},
    ...
    if (live) {
        ...
        const client = new __1.StarWarsSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['STAR_WARS_TEST_SPECIES_ENTID'];
    ...
    const client = new __1.StarWarsSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=SpeciesDirect.test.js.map
...
(0, node_test_1.describe)('VehicleDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-vehicle', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-vehicle', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['STAR_WARS_TEST_VEHICLE_ENTID'];

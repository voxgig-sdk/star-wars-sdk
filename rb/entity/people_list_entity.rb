...
class PeopleListEntity
  ...
  def make
    opts = @_entopts.dup
    PeopleListEntity.new(@_client, opts)
  end
  ...
  # @return [PeopleList, Hash] the current PeopleList data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of PeopleList fields)
  def match_get

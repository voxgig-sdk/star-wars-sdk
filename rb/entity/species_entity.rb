...
class SpeciesEntity
  ...
  def make
    opts = @_entopts.dup
    SpeciesEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = StarWarsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Species, Hash] the current Species data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = StarWarsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Species fields)
  def match_get
  ...
  #
  # @param reqmatch [SpeciesLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Species.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Species, Hash] the loaded Species; raises StarWarsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = StarWarsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [SpeciesListMatch, Hash, nil] match filter (any subset of
  #   Species fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Species>, Array] the matching Species items; raises StarWarsError on failure

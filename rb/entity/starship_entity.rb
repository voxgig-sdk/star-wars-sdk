...
class StarshipEntity
  ...
  def make
    opts = @_entopts.dup
    StarshipEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Starship.load works with no args).
  ...
  #
  # @param reqmatch [StarshipListMatch, Hash, nil] match filter (any subset of
  #   Starship fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control

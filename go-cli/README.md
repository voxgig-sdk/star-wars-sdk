...
boru-driven command-line client **and** interactive REPL for the StarWars
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/star-wars-cli)
make build
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/star-wars-cli
   ```
...
```sh
./star-wars-cli load 1 film          # scalar shorthand for {id:1}
./star-wars-cli load '{id:1}' film     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `star-wars>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `film`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/star-wars-cli`. |
...
(`./star-wars-cli <expr>`) and interactively (the REPL), and expressions compose the

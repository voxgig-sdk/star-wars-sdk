...
module StarWarsUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = StarWarsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = StarWarsResponse.new(fetched)
    else

...
MCP server exposing the StarWars SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `star-wars_list` | `entity`, optional `query` map | First page of records as JSON |
| `star-wars_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `star-wars_list` and
`star-wars_load` tools become available in new sessions.

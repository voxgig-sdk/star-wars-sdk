...
[MCP](https://modelcontextprotocol.io) server exposing the StarWars SDK as
two agent tools — `star-wars_list` and `star-wars_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/star-wars-mcp)
make build

# 2. Provide credentials via the environment
export STAR_WARS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user star-wars \
  -- /absolute/path/to/star-wars-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./star-wars-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// star-wars_list: first page of records
{ "entity": "film" }
{ "entity": "film", "query": { } }

// star-wars_load: one record by id
{ "entity": "film", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/star-wars-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user star-wars \
     -- "$PWD"/dist/*/star-wars-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `star-wars_list` and `star-wars_load` tools now appear
   in new sessions. Ask the agent to *"list film using star-wars"*
   and it calls `star-wars_list` with `{"entity":"film"}`.
...
```sh
export STAR_WARS_APIKEY=sk_live_xxx            # API key
export STAR_WARS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `star-wars_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `star-wars_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `STAR_WARS_APIKEY` | API key sent with every request. |
| `STAR_WARS_BASE` | Optional override of the API base URL. |
...
```sh
./star-wars-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"star-wars_load","arguments":{"entity":"film","query":{"id":1}}}}'
```
